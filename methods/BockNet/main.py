...
class Trainer(object):
    ...
    def train_epoch(self) -> None:
        ...
        if ((self.epoch + 1) % self.save_freq == 0):
            torch.save(self.model.state_dict(), os.path.join(self.model_path, 'BockNet' + '_' + file + '_' + str(self.epoch + 1) + '.pkl'))
...
def train_model(opt,file):
    
    expr_dir = os.path.join('./checkpoints/', file)
    ...
    loader_train, band = BockNetData(opt,file)
...
def predict(opt,file):
    ...
    expr_dirs = os.path.join('./result/', file)
    if not os.path.exists(expr_dirs):
        os.makedirs(expr_dirs)
    
    log_output = open(f"{expr_dirs}/log.txt", 'w')
    
    model_weights = os.path.join(model_path, 'BockNet' + '_' + file + '_' + str(opt.epochs) + '.pkl')
    ...
    for file_test in file_name:
        # test datalodar
        data_dir = '../../data/'
        image_file = data_dir + file_test + '.mat'
        
        save_subdir = os.path.join(save_dir, "BockNet", file)
        ...
        roc_auc = roc_auc_score(gt.flatten(), detectmap.flatten())
        print('Auc: %.4f' % roc_auc)

        fpr, tpr, thre = roc_curve(gt.flatten(), detectmap.flatten())
        map_path = os.path.join(save_subdir, "BockNet_map_"+file_test+".mat")
        sio.savemat(map_path, {'show': detectmap})
        roc_path = os.path.join(save_subdir, "BockNet_roc_"+file_test+".mat")
